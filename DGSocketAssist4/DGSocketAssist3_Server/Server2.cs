...
namespace DGSocketAssist3_Server
{
	...
	public class Server2
	{
			...
			, byte[] byteMessage)
		{
			...
			{
				this.OnMessaged(sender, byteMessage);
			}
		}
		#endregion
		...
		/// <param name="byteMessage"></param>
		/// <exception cref="NotImplementedException"></exception>
		private void NewUser_OnMessaged(ClientListener sender, byte[] byteMessage)
		{
			this.MessagedCall(sender, byteMessage);
		}
		...
		public void AllMessage(byte[] byteMessage)
		{
			foreach (ClientListener itemCL in this.ClientList)
			{
				itemCL.Send(byteMessage);
			}//end foreach itemCL
		}
	}
}
